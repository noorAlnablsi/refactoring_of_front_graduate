...
function ExamSummarySidebar({
  ...
  const audience = getSurveyAudienceScope(test || { audience_scope: draft?.audience_scope })
      ...
      <aside className="rounded-2xl bg-white p-5 ring-1 ring-[#E5E9EB]">
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-extrabold text-[#2A3433]">
            {isSurvey ? t('wizard.sidebar.summary', { ns: 'surveys' }) : t('sidebar.examSummary')}
          </h3>
        ...
        {getTestName(test) || draft?.name ? (
          <div className="mt-5 border-t border-[#E5E9EB] pt-4">
            <p className="text-xs font-semibold text-[#94A3B8]">
              {isSurvey ? t('wizard.review.surveyName', { ns: 'surveys' }) : t('sidebar.examName')}
            </p>
