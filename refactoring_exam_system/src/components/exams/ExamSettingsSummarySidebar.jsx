...
function ExamSettingsSummarySidebar({ test, settings, isSurvey = false }) {
    ...
    <div className="space-y-4">
      <aside className="rounded-2xl bg-white p-5 ring-1 ring-[#E5E9EB]">
        <h3 className="text-sm font-extrabold text-[#2A3433]">
          {isSurvey ? t('wizard.sidebar.summary', { ns: 'surveys' }) : t('wizard.summary.settingsTitle')}
        </h3>

        <dl className="mt-5 space-y-4 text-sm">
          <div>
            <dt className="text-xs font-semibold text-[#94A3B8]">
              {isSurvey ? t('wizard.review.surveyName', { ns: 'surveys' }) : t('wizard.summary.examName')}
            </dt>
          ...
          {isSurvey ? (
            <div className="flex items-center justify-between gap-3">
              <dt className="text-xs font-semibold text-[#94A3B8]">
                {t('wizard.sidebar.audience', { ns: 'surveys' })}
              </dt>
              <dd className="font-extrabold text-[#2A3433]">
                {t(getSurveyAudienceI18nKey(audience), { ns: 'surveys' })}
              </dd>
            </div>
          ) : (
            <>
