...
function ExamSettingsSummarySidebar({ test, settings }) {
  ...
  const monitoringActive = isAiMonitoringActive(settings)

  return (
    <div className="space-y-4">
      <aside className="rounded-2xl bg-white p-5 ring-1 ring-[#E5E9EB]">
        <h3 className="text-sm font-extrabold text-[#2A3433]">ملخص الإعدادات</h3>

        <dl className="mt-5 space-y-4 text-sm">
          <div>
            <dt className="text-xs font-semibold text-[#94A3B8]">اسم الامتحان</dt>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <dd className="font-extrabold text-[#2A3433]">{questionsCount} سؤال</dd>
          </div>
          <div className="flex items-center justify-between gap-3">
            <dt className="text-xs font-semibold text-[#94A3B8]">إجمالي الدرجات</dt>
            <dd className="font-extrabold text-[#2A3433]">{totalScore} درجة</dd>
          </div>
          <div className="flex items-center justify-between gap-3">
            <dt className="text-xs font-semibold text-[#94A3B8]">نظام المراقبة</dt>
            <dd>
              <span
      ...
      <div className="rounded-2xl bg-[#E8F7F6] p-4 ring-1 ring-[#CFECE9]">
        <div className="flex items-start gap-3">
          ...
          <p className="text-xs leading-6 text-[#374151]">
            منع النسخ واللصق يقلل فرص تسريب الأسئلة بنسبة 40% وفقاً لإحصائيات المنصة.
          </p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-[#0F766E] via-[#2AA8A2] to-[#5EEAD4] p-5 text-white shadow-[0_12px_30px_rgba(42,168,162,0.28)]">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-extrabold">تأمين الذكاء الاصطناعي</p>
        </div>
        ...
        <p className="mt-4 text-center text-xs font-semibold text-white/90">نظام مراقبة نشط</p>
      </div>
    </div>
  )
}
